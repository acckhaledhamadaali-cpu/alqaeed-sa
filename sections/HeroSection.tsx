            ...
            <header id="hero-header" className="flex flex-col items-center md:items-start space-y-3.5 md:space-y-4 w-full text-center md:text-right">
              <div id="hero-brand-area" className="flex flex-col items-center md:items-start space-y-1">
                <h1 id="hero-brand-name" className={`${TYPOGRAPHY.heading.h1} font-bold text-text-primary`}>
                  <span id="hero-brand-name-text">القائد</span>
                  <span id="hero-brand-tagline" className={`block ${TYPOGRAPHY.body.large} text-secondary font-semibold mt-1`}>
                    محاسب مالي عن بُعد
                  </span>
                </h1>
              </div>

              <h2 id="hero-main-message" className={`${TYPOGRAPHY.heading.h2} font-bold text-text-primary max-w-2xl leading-relaxed`}>
                يساعدك على فهم أرقام منشأتك
                <span className="block">واتخاذ قرارات أفضل</span>
              </h2>

              <p id="hero-description" className={`${TYPOGRAPHY.body.large} max-w-xl leading-relaxed`}>
                أنظم حسابات منشأتك
                <span className="block">وأعد التقارير والقوائم المالية</span>
                <span className="block">وأحلل الربحية والسيولة والتدفقات النقدية</span>
              </p>

              <p id="hero-trust-line" className="text-xs md:text-sm text-text-secondary font-arabic font-normal leading-relaxed mt-1">
